...
def create_combinations(items, size):
    item_combinations = list(combinations(items, size))
    return [tuple(item for subtuple in combination for item in subtuple) for combination in item_combinations]
# def create_combinations(itemsets, size):
#     candidate_itemsets = []
#     for i in range(len(itemsets)):
#         for j in range(i + 1, len(itemsets)):
#             itemset1 = itemsets[i]
#             itemset2 = itemsets[j]
#             if itemset1[:size - 2] == itemset2[:size - 2]:
...
def count_itemset_subset(itemset):
    grouped_bcast = grouped_data_RDD
    return sum(1 for x in grouped_bcast if set(itemset).issubset(x[1]))
...
while size <= length:
    # comp_combinations = create_combinations(singletons, size)
    print(frequent_items)
    comp_combinations = create_combinations(frequent_items, size)
    print(comp_combinations)
    frequent_RDD = frequent_items_RDD.flatMap(lambda x: comp_combinations).distinct().sortBy(lambda a: a)
